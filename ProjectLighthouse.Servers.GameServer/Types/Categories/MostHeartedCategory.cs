...
public class MostHeartedCategory : Category
{
    ...
    public override SlotEntity? GetPreviewSlot(DatabaseContext database) => database.Slots.Where(s => s.Type == SlotType.User).AsEnumerable().MaxBy(s => s.Hearts(database));
    public override IEnumerable<SlotEntity> GetSlots
        ...
        => database.Slots.ByGameVersion(GameVersion.LittleBigPlanet3, false, true)
            .AsEnumerable()
            .OrderByDescending(s => s.Hearts(database))
