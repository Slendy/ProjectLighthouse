...
public static class SMTPHelper
{
    ...
    public static void SendRegistrationEmail(IMailService mail, ServerConfiguration serverConfiguration, UserEntity user)
    {
        string body = "An account for Project Lighthouse has been registered with this email address.\n\n" +
                      $"You can login at {serverConfiguration.ExternalUrl}.";

        mail.SendEmail(user.EmailAddress, "Project Lighthouse Account Created: " + user.Username, body);
    }

    public static async Task<bool> SendVerificationEmail(DatabaseContext database, IMailService mail, ServerConfiguration serverConfiguration, UserEntity user)
    {
        ...
        string body = "Hello,\n\n" +
                      ...
                      $"To verify your account, click the following link: {serverConfiguration.ExternalUrl}/verifyEmail?token={verifyToken.EmailToken}\n\n\n" +
