...
public partial class DatabaseContext
{
    ...
    private UserEntity? UserFromLighthouseToken(string lighthouseToken)
    {
        ...
        if (DateTimeOffset.UtcNow <= token.ExpiresAt) return this.Users.FirstOrDefault(u => u.UserId == token.UserId);
    ...
    public WebTokenEntity? WebTokenFromRequest(HttpRequest request)
    {
        ...
        if (DateTimeOffset.UtcNow <= token.ExpiresAt) return token;
    ...
    public async Task<UserEntity?> UserFromPasswordResetToken(string? resetToken)
    {
        ...
        if (token.Created >= DateTimeOffset.UtcNow.AddHours(-1))
    ...
    public bool IsRegistrationTokenValid(string? tokenString)
    {
        ...
        if (token.Created >= DateTimeOffset.UtcNow.AddDays(-7)) return true;
    ...
    {
        List<GameTokenEntity> expiredTokens = await this.GameTokens.Where(t => DateTimeOffset.UtcNow > t.ExpiresAt).ToListAsync(); 
        ...
        await this.GameTokens.RemoveWhere(t => DateTimeOffset.UtcNow > t.ExpiresAt);
        await this.WebTokens.RemoveWhere(t => DateTimeOffset.UtcNow > t.ExpiresAt);
        await this.EmailVerificationTokens.RemoveWhere(t => DateTimeOffset.UtcNow > t.ExpiresAt);
        await this.EmailSetTokens.RemoveWhere(t => DateTimeOffset.UtcNow > t.ExpiresAt);
    }
